...
def calcular_frequencias(s):
    dicionario = {}
    for caractere in s:
        if caractere not in dicionario.keys():
...
def gerar_arvore_de_huffman(s):
    dicionario_folhas = calcular_frequencias(s)
    folhas = []

    for folha in dicionario_folhas:
        folhas.append(Folha(folha,dicionario_folhas[folha]))
...
class Arvore(object):
    ...
    def decodificar(self, codigo):
        ...
        for valor in dicionario:
            novoDicionario[dicionario[valor]] = valor


        for num_bin in codigo:
            codigo_letra+=num_bin
            if codigo_letra in novoDicionario:
                palavra+=novoDicionario[codigo_letra]
                codigo_letra = ""
